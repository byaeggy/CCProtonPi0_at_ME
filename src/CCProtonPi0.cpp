...
StatusCode CCProtonPi0::initialize()
{
    ...
    declareIntTruthBranch("N_chargedPion", -1 );
    ...
    */
    ...
    // Extra Energy 
    declareDoubleEventBranch("Extra_Energy_Dispersed", SENTINEL );//Total
    declareDoubleEventBranch("Extra_Energy_Muon", SENTINEL );
    declareDoubleEventBranch("Extra_Energy_Rejected", SENTINEL );
    declareDoubleEventBranch("Extra_Evis_Leftover", SENTINEL); //Energy
    // Primary Vertex
...
StatusCode CCProtonPi0::reconstructEvent( Minerva::PhysicsEvent *event, Minerva::GenMinInteraction* truthEvent ) const
{
    ...
    //SetExtraEnergyTotal(event);  //Roger commented
...
StatusCode CCProtonPi0::interpretEvent( const Minerva::PhysicsEvent *event, const Minerva::GenMinInteraction *truthEvent, NeutrinoVect& interaction_hyp ) const
{
    ...
    debug()<<"Finished hadronic recoil energy"<<endmsg;
...
StatusCode CCProtonPi0::tagTruth( Minerva::GenMinInteraction* truthEvent ) const 
{
    ...
    debug() << "Event tagged as RECONSTRUCTABLE" << endmsg;
    return StatusCode::SUCCESS;
}
...
//==============================================================================
// Set Pion particle data
//==============================================================================
bool CCProtonPi0::setPionData(Minerva::PhysicsEvent* event) const
{
    if ( m_PionParticles.size() == 0){
        warning() << "m_PionParticles is empty! Exiting..." << endmsg;
        return false;
    }

    if (m_PionProngs.size() != m_PionParticles.size()){
        warning() << "m_PionProngs and m_PionParticles NOT SAME SIZE" << endmsg;
        return false;
    }
    ...
    // DeclareVariables
    int leadingPionIndice = 0;
    double tempPionMomentum = 0.0;
    ...
    std::vector<double> pion_theta(10,SENTINEL);
    std::vector<double> pion_thetaX(10,SENTINEL);
    std::vector<double> pion_thetaY(10,SENTINEL);
    std::vector<double> pion_phi(10,SENTINEL);
    ...
    std::vector<double> pion_end_x(10,SENTINEL);
    std::vector<double> pion_end_y(10,SENTINEL);
    std::vector<double> pion_end_z(10,SENTINEL);

    std::vector<double> pion_start_x(10,SENTINEL);
    std::vector<double> pion_start_y(10,SENTINEL);
    std::vector<double> pion_start_z(10,SENTINEL);
    ...
    for(unsigned int i = 0; i < m_PionProngs.size(); i++) {
      SmartRef<Minerva::Prong> prong = m_PionProngs[i];
      SmartRef<Minerva::Particle> particle = m_PionParticles[i];

      double theta = prong->minervaTracks().front()->theta();
      double phi   = prong->minervaTracks().front()->phi();

      pion_theta[i]  = m_coordSysTool->thetaWRTBeam(particle->momentumVec());
      pion_thetaX[i] = m_coordSysTool->thetaXWRTBeam(particle->momentumVec());
      pion_thetaY[i] = m_coordSysTool->thetaYWRTBeam(particle->momentumVec());
      pion_phi[i]    = m_coordSysTool->phiWRTBeam(particle->momentumVec());

      Gaudi::LorentzVector dEdXpionfourVec;
      StatusCode sc = m_energyCorrectionTool->getCorrectedEnergy(prong,particle,vertexZ,dEdXpionfourVec);
      if( sc ) particle->setMomentumVec(dEdXpionfourVec);
      else dEdXpionfourVec = particle->momentumVec();

      p_dedx[i] = sqrt( std::pow(dEdXpionfourVec.E(),2) - std::pow(MinervaUnits::M_pion,2) );
      correctPionProngEnergy(prong, p_calCorrection[i], p_visEnergyCorrection[i]);

      E[i]  = sqrt( std::pow(p_calCorrection[i],2) + std::pow(MinervaUnits::M_pion,2) );
      ...
      Gaudi::LorentzVector pionfourVec(px[i],py[i],pz[i],E[i]);

      ekin[i] = pionfourVec.E() - MinervaUnits::M_pion;

      pion_end_x[i]   = (prong->minervaTracks().back())->lastState().x();
      pion_end_y[i]   = (prong->minervaTracks().back())->lastState().y();
      pion_end_z[i]   = (prong->minervaTracks().back())->lastState().z();

      pion_start_x[i] = (prong->minervaTracks().back())->firstState().x();
      pion_start_y[i] = (prong->minervaTracks().back())->firstState().y();
      pion_start_z[i] = (prong->minervaTracks().back())->firstState().z();

      length[i]  = calcDistance( pion_end_x[i], pion_end_y[i], pion_end_z[i], pion_start_x[i], pion_start_y[i], pion_start_z[i]);
      ...
      pionScoreLLR[i] = particle->score();
        // Phil's Likelihood Method for PID
      /*
        if(!pionLikelihood.empty()){
	pionScoreLLR[i] = pionLikelihood[i]->score();
        }else{
	debug()<<"particlesLikelihood is empty!"<<endmsg;
        }
      */
        // Find Interaction Pion -- Highest Momentum Pion
      if (p[i]  > tempPionMomentum){
	tempPionMomentum = p[i];
	leadingPionIndice = i;
      }

      //----------------------------------------------------------------------
      // Debugging: Check values
      debug()<<"Pion Candidate = "     << i<<" P = "        << p[i]       << endmsg;
      debug()<<"P4(Pion) = ( "         << px[i]<<", "       << py[i]      << ", "     << pz[i]<<", "<< E[i] <<" )"<< endmsg;
      //debug()<<"  protonScore = "      << protonScore[i]    << " pionScore = "<< pionScore[i] << endmsg;
      debug()<<"  pionScoreLLR = "     << pionScoreLLR[i]   << endmsg;
      debug()<<"  length[i] = "        << length[i]         << endmsg;
      debug()<<"  ekin[i] = "          << ekin[i]           << endmsg;
      debug()<<"  kinked[i] = "        << kinked[i]         << endmsg;
      debug()<<"  odMatch[i] = "       << odMatch[i]        << endmsg;
      debug()<<"  leadingPionIndice = "<< leadingPionIndice << endmsg;
      //----------------------------------------------------------------------

      // Check Momentum
      if ( isnan(p[i]) ) return false;
    }// end of pion candidates

    // Set Leading Pion 4-Momentum
    m_pion_4P.SetPxPyPzE( px[leadingPionIndice], py[leadingPionIndice], pz[leadingPionIndice], E[leadingPionIndice]);

    event->setContainerDoubleData("all_pions_p_calCorrection",p_calCorrection);
    event->setContainerDoubleData("all_pions_p_visEnergy",p_visEnergyCorrection);
    event->setContainerDoubleData("all_pions_p_dEdXTool",p_dedx);
    event->setContainerDoubleData("all_pions_endPointX",pion_end_x);
    event->setContainerDoubleData("all_pions_endPointY",pion_end_y);
    event->setContainerDoubleData("all_pions_endPointZ",pion_end_z);
    event->setContainerDoubleData("all_pions_startPointX",pion_start_x);
    event->setContainerDoubleData("all_pions_startPointY",pion_start_y);
    event->setContainerDoubleData("all_pions_startPointZ",pion_start_z);
    event->setContainerDoubleData("all_pions_length", length);
    event->setContainerDoubleData("all_pions_px",px);
    event->setContainerDoubleData("all_pions_py",py);
    event->setContainerDoubleData("all_pions_pz",pz);
    event->setContainerDoubleData("all_pions_E", E);
    event->setContainerDoubleData("all_pions_P", p);
    event->setContainerDoubleData("all_pions_KE", ekin);
    event->setContainerIntData("all_pions_kinked", kinked);
    event->setContainerIntData("all_pions_odMatch", odMatch);
    //event->setContainerDoubleData("all_pions_protonScore",protonScore);
    //event->setContainerDoubleData("all_pions_pionScore",pionScore);
    event->setContainerDoubleData("all_pions_LLRScore", pionScoreLLR);
    event->setContainerDoubleData("all_pions_chi2_ndf", chi2);
    event->setContainerDoubleData("all_pions_theta", pion_theta);
    event->setContainerDoubleData("all_pions_thetaX", pion_thetaX);
    event->setContainerDoubleData("all_pions_thetaY", pion_thetaY);
    event->setContainerDoubleData("all_pions_phi", pion_phi);

    event->setDoubleData("pion_px", px[leadingPionIndice]);
    event->setDoubleData("pion_py", py[leadingPionIndice]);
    event->setDoubleData("pion_pz", pz[leadingPionIndice]);
    event->setDoubleData("pion_E", E[leadingPionIndice]);
    event->setDoubleData("pion_P", p[leadingPionIndice]);
    event->setDoubleData("pion_KE",ekin[leadingPionIndice]);
    event->setDoubleData("pion_theta", pion_theta[leadingPionIndice]);
    event->setDoubleData("pion_phi", pion_phi[leadingPionIndice]);
    event->setDoubleData("pion_length", length[leadingPionIndice]);
    //event->setDoubleData("pion_protonScore", protonScore[leadingPionIndice]);
    //event->setDoubleData("pion_pionScore", pionScore[leadingPionIndice]);
    event->setDoubleData("pion_LLRScore", pionScoreLLR[leadingPionIndice]);
    event->setIntData("pion_kinked",kinked[leadingPionIndice]);
    event->setIntData("pion_leadingIndice",leadingPionIndice);
...
*/
    ...
    for(unsigned int j = 0; j < m_dedx_uncertainties.size(); j++) {
        ...
        debug()<<" proton_energy_shift_"<<name<<" = "<<(dedxMapMomentum.find(name))->second.at(leadingProtonIndice)<<endmsg;
...
bool CCProtonPi0::setPi0Data( Minerva::PhysicsEvent *event ) const 
{
    ...
    */
    ...
    double pi0_E = sqrt(pi0_P*pi0_P + invMass*invMass);
    double pi0_KE = pi0_E - invMass;
...
*/  
    //End of Roger's commentted lines 
...
void CCProtonPi0::Calculate_dEdx( Minerva::PhysicsEvent* event, const Minerva::IDBlob* blob,  unsigned int blob_number) const
{
    ...
    debug() << "Calculate dEdx for blob: " << blob_number <<  " (forward,backward) (" << (blob->direction().Z() > 0) << "," << (blob->direction().Z() < 0) << ")" << endmsg;
...
bool CCProtonPi0::PreFilterPi0(Minerva::PhysicsEvent *event, Minerva::GenMinInteraction* truthEvent ) const
{
    ...
    const double max_Evis_Other = 50000; // energy bigger than 2500 MeV must be ignored - DIS Events
...
std::pair<int,double> CCProtonPi0::OneParLineFitBlob(const Minerva::IDBlob* blob, Minerva::PhysicsEvent *event) const
{
    ...
    for (double dz = zmin; dz <= zmax; dz += zstep) {
        ...
        int _errno = minuit->Minimize();
        if (_errno > 0) {
            std::cerr << "Fitting error: " << _errno << std::endl;
...
double CCProtonPi0::TwoParLineFitBlobVtxDistance(const Minerva::IDBlob* blob, Minerva::PhysicsEvent *event) const
{
    ...
    if (_errno > 0) {
        std::cerr << "TwoParLineFit:: fitting error: " << _errno << std::endl;
    }
