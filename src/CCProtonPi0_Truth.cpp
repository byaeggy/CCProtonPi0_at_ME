...
bool CCProtonPi0::tagSignal(Minerva::GenMinInteraction* truthEvent) const
{
    ...
    for (it_mcpart = pri_trajectories.begin(); it_mcpart != pri_trajectories.end(); ++it_mcpart) {
     ...
     else if ( particle_PDG == PDG::pi || particle_PDG == -PDG::pi) N_chargedPion++;
        else{
            N_other++;
    ...
    debug()<<"N(Proton) = "<<N_proton<<" N(Neutron) = "<<N_neutron<<" N(Pi0) = " <<N_pi0
	   << " N(Charged Pion) = "<<N_chargedPion<<" N(Other) = "<<N_other<<endmsg;
    ...
    truthEvent->setIntData("N_chargedPion", N_chargedPion);
    ...
    bool isSignal = isCCNeutrino && isFSGood && isAcceptanceGood; 
